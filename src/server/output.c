...
static bool output_set_cursor(struct wlr_output *wlr_output, struct wlr_buffer *buffer, int hotspot_x, int hotspot_y) {
    ...
    output->server->wsland_cursor.hotspot_x = hotspot_x;
    output->server->wsland_cursor.hotspot_y = hotspot_y;
    output->server->wsland_cursor.dirty = true;

    wl_signal_emit(&output->server->events.wsland_cursor_frame, output);
    return true;
}
